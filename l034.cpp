...
void part4(){
    ...
    long long end = (long long)(points.size());
    for(long long i = 0; i < points.size(); i++){
        long long swapIndex = (long long)((double) rand() / RAND_MAX * (end-i)) + i;
        Point temp;
        temp.setX(points[i].getX());
        temp.setY(points[i].getY());
        Point swap;
        swap.setX(points[swapIndex].getX());
        swap.setY(points[swapIndex].getY());
        points[i] = swap;
        points[swapIndex] = temp;
    }
    unordered_map<pair<unsigned long long, unsigned long long>, Point, MyHashFunction> map;
    Segment min;
    min.setDist(distance(points[0].getX(), points[0].getY(), points[1].getX(), points[1].getY()) / 2);
    min.setA(points[0]);
    min.setB(points[1]);
    int remaps = 0;
    for(int i = 0; i < points.size(); i++){
        unsigned long long row = (unsigned long long) (points[i].getY() / min.getDist());
        unsigned long long col = (unsigned long long) (points[i].getX() / min.getDist());
        bool passes = true;
        Point test;
        for(int a = -2; a <= 2; a++){
            for(int b = -2; b <= 2; b++){
                ...
                if(map.find(temp) != map.end()){
                    Point second = map[temp];
                    if(distance(points[i].getX(), points[i].getY(), second.getX(), second.getY()) / 2 < min.getDist()){
                        min.setDist(distance(points[i].getX(), points[i].getY(), second.getX(), second.getY()) / 2);
                        min.setA(points[i]);
                        min.setB(second);
                        passes = false;
                    }
        ...
        if(passes){
            ...
            remaps++;
            for(int j = 0; j <= i; j++){
                unsigned long long r = (unsigned long long) (points[j].getY() / min.getDist());
                unsigned long long c = (unsigned long long) (points[j].getX() / min.getDist());
                pair<unsigned long long, unsigned long long> k;
                k.first = r;
                k.second = c;
                map[k] = points[j];
    ...
    min.setDist(min.getDist() * 2);
    cout << "Randomized" << "\n";
    myfile << "Randomized" << "\n";
    cout << "Point 1: (" << min.getA().getX() << ", " << min.getA().getY() << ")" << "\n";
    cout << "Point 2: (" << min.getB().getX() << ", " << min.getB().getY() << ")" << "\n";
    cout << "Min Distance: " << min.getDist() << "\n";
    cout << "remaps: " << remaps << "\n";
    cout << "Time difference = " << chrono::duration_cast<chrono::microseconds>(endTime - begin).count() << " microseconds" << "\n";
    myfile << "Point 1: (" << min.getA().getX() << ", " << min.getA().getY() << ")" << "\n";
    myfile << "Point 2: (" << min.getB().getX() << ", " << min.getB().getY() << ")" << "\n";
    myfile << "Min Distance: " << min.getDist() << "\n";
    myfile << "Time difference = " << chrono::duration_cast<chrono::microseconds>(endTime - begin).count() << " microseconds" << "\n";
    drawCircle(min.getA().getX(), min.getA().getY(), 2, false);
    drawCircle(min.getA().getX(), min.getA().getY(), 3, false);
    drawCircle(min.getB().getX(), min.getB().getY(), 2, false);
    drawCircle(min.getB().getX(), min.getB().getY(), 3, false);
    convertPPMToFile();
}
...
void part3(){
    ...
    myfile << "Complete Recursive" << "\n";
    cout << "Point 1: (" << segment.getA().getX() << ", " << segment.getA().getY() << ")" << "\n";
    cout << "Point 2: (" << segment.getB().getX() << ", " << segment.getB().getY() << ")" << "\n";
    cout << "Min Distance: " << segment.getDist() << "\n";
    cout << "Time difference = " << chrono::duration_cast<chrono::microseconds>(end - begin).count() << " microseconds" << "\n";
    myfile << "Point 1: (" << segment.getA().getX() << ", " << segment.getA().getY() << ")" << "\n";
    myfile << "Point 2: (" << segment.getB().getX() << ", " << segment.getB().getY() << ")" << "\n";
    myfile << "Min Distance: " << segment.getDist() << "\n";
